...
filter_to_test = 'IMM';  % Options: 'KF', 'EKF-CV', 'EKF-CA', 'UKF'
...
for i = 1:num_q_values
    ...
    try
        ...
        % Store results - FIXED FIELD NAMES
        results.pos_rmse(i) = stats.posRMSE;    % Changed from position_rmse
        results.vel_rmse(i) = stats.velRMSE;    % Changed from velocity_rmse  
        results.sog_rmse(i) = stats.sogRMSE;    % Changed from sog_rmse
        results.cog_rmse(i) = stats.cogRMSE;    % Changed from cog_rmse
...
subplot(2, 2, 1);
plot(q_values, results.pos_rmse, 'b-o', 'LineWidth', 2, 'MarkerSize', 6);
xlabel('Process Noise Intensity (q)');
ylabel('Position RMSE (m)');
title('Position RMSE vs Process Noise');
grid on;
hold on;
plot(q_values(min_pos_idx), min_pos_rmse, 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r');
legend('RMSE', 'Optimal q', 'Location', 'best');

% Plot velocity RMSE vs q
subplot(2, 2, 2);
plot(q_values, results.vel_rmse, 'g-o', 'LineWidth', 2, 'MarkerSize', 6);
xlabel('Process Noise Intensity (q)');
ylabel('Velocity RMSE (m/s)');
title('Velocity RMSE vs Process Noise');
grid on;
hold on;
plot(q_values(min_vel_idx), min_vel_rmse, 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r');
legend('RMSE', 'Optimal q', 'Location', 'best');

% Plot SOG RMSE vs q
subplot(2, 2, 3);
plot(q_values, results.sog_rmse, 'k-o', 'LineWidth', 2, 'MarkerSize', 6);
xlabel('Process Noise Intensity (q)');
ylabel('SOG RMSE (m/s)');
title('SOG RMSE vs Process Noise');
grid on;
hold on;
plot(q_values(min_sog_idx), min_sog_rmse, 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r');
legend('RMSE', 'Optimal q', 'Location', 'best');

% Plot COG RMSE vs q
subplot(2, 2, 4);
plot(q_values, results.cog_rmse, 'm-o', 'LineWidth', 2, 'MarkerSize', 6);
xlabel('Process Noise Intensity (q)');
ylabel('COG RMSE (degrees)');
title('COG RMSE vs Process Noise');
grid on;
hold on;
plot(q_values(min_cog_idx), min_cog_rmse, 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r');
legend('RMSE', 'Optimal q', 'Location', 'best');
...
figure;
if ~isnan(optimal_q)
    plot(q_values, combined_score, 'r-o', 'LineWidth', 2, 'MarkerSize', 6);
    hold on;
    plot(optimal_q, min_combined_score, 'ko', 'MarkerSize', 10, 'MarkerFaceColor', 'k');
    xlabel('Process Noise Intensity (q)');
