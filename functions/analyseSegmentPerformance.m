...
function analyseSegmentPerformance(data, kf_est, ekf_est, ukf_est, imm_est)
...
pos_rmse = zeros(4, numSegments);  % [KF; EKF; UKF; IMM] x numSegments
vel_rmse = zeros(4, numSegments);
sog_rmse = zeros(4, numSegments);
cog_rmse = zeros(4, numSegments);
...
for i = 1:numSegments
    ...
    pos_rmse(2, i) = sqrt(mean((sqrt((ekf_est.x_est(segIdx) - data.x_true(segIdx)).^2 + ...
                                     (ekf_est.y_est(segIdx) - data.y_true(segIdx)).^2)).^2));
    pos_rmse(3, i) = sqrt(mean((sqrt((ukf_est.x_est(segIdx) - data.x_true(segIdx)).^2 + ...
                                     (ukf_est.y_est(segIdx) - data.y_true(segIdx)).^2)).^2));
    pos_rmse(4, i) = sqrt(mean((sqrt((imm_est.x_est(segIdx) - data.x_true(segIdx)).^2 + ...
    ...
    vel_rmse(2, i) = sqrt(mean((sqrt((ekf_est.vx_est(segIdx) - data.vx_true(segIdx)).^2 + ...
                                     (ekf_est.vy_est(segIdx) - data.vy_true(segIdx)).^2)).^2));
    vel_rmse(3, i) = sqrt(mean((sqrt((ukf_est.vx_est(segIdx) - data.vx_true(segIdx)).^2 + ...
                                     (ukf_est.vy_est(segIdx) - data.vy_true(segIdx)).^2)).^2));
    vel_rmse(4, i) = sqrt(mean((sqrt((imm_est.vx_est(segIdx) - data.vx_true(segIdx)).^2 + ...
                                     (imm_est.vy_est(segIdx) - data.vy_true(segIdx)).^2)).^2));
    
    % SOG RMSE
    sog_rmse(1, i) = sqrt(mean((kf_est.sog_est(segIdx) - data.sog_true(segIdx)).^2));
    sog_rmse(2, i) = sqrt(mean((ekf_est.sog_est(segIdx) - data.sog_true(segIdx)).^2));
    sog_rmse(3, i) = sqrt(mean((ukf_est.sog_est(segIdx) - data.sog_true(segIdx)).^2));
    sog_rmse(4, i) = sqrt(mean((imm_est.sog_est(segIdx) - data.sog_true(segIdx)).^2));
...
legend('KF', 'EKF', 'UKF', 'IMM', 'Location', 'best');
grid on;
...
for i = 1:numSegments
    ...
    fprintf('%-10s | %-10.2f | %-10.2f | %-10.2f | %-10.2f\n', ...
            'IMM', pos_rmse(4,i), vel_rmse(4,i), sog_rmse(4,i), cog_rmse(4,i));
    ...
    fprintf('Position: %s (%.2f m)\n', ...
            {'KF', 'EKF', 'UKF', 'IMM'}{bestPosIdx}, pos_rmse(bestPosIdx,i));
    fprintf('Velocity: %s (%.2f m/s)\n', ...
            {'KF', 'EKF', 'UKF', 'IMM'}{bestVelIdx}, vel_rmse(bestVelIdx,i));
    fprintf('SOG: %s (%.2f m/s)\n', ...
            {'KF', 'EKF', 'UKF', 'IMM'}{bestSogIdx}, sog_rmse(bestSogIdx,i));
    fprintf('COG: %s (%.2f degrees)\n', ...
            {'KF', 'EKF', 'UKF', 'IMM'}{bestCogIdx}, cog_rmse(bestCogIdx,i));
end
...
for i = 1:numSegments
    ...
    fprintf('Segment %d (%s): Best filter is %s\n', ...
            segments(i), segment_names{i}, {'KF', 'EKF', 'UKF', 'IMM'}{bestIdx});
end
