...
% RUNUKFCV - Apply Unscented Kalman Filter (CV model) to AIS data with NEES/NIS
if nargin < 4, vel_std = 1; end
if nargin < 3, pos_std = 15; end
if nargin < 2, q = 0.1; end

% fprintf('Starting Unscented Kalman Filter (CV) with q=%.3f, pos_std=%.1fm, vel_std=%.1fm/s\n', q, pos_std, vel_std);

% Run filter with innovation tracking
[x_est, P_est, innovations, S_innovations] = ukfCV(data, q, pos_std, vel_std);
...
stats = calculateFilterStats(estimates, data, 'UKF-CV', P_est, innovations, S_innovations);
